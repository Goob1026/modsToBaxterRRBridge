#!/usr/bin/env python
#Kim Oakes added measuring inner loop commanded position and additional torque values Dec 14, 2020
import roslib
...
property double[] commanded_position
property double[] all_gravity_torque
property double[] hysteresis_torque
property double[] hysteresis_state
property double[] commanded_torque
property double[] crosstalk_torque
...
class BaxterPeripherals_impl(object):
    ...
    # hysteresis info functions                                            
    @property
    def hysteresis_torque(self):
        return self._hysteresis_torque.values()                                            

    def hyst_callback(self, data):
        with self._hyst_lock:
            if data.gravity_model_effort:
                for n in xrange(0,len(data.name)):
                    self._hysteresis_torque[data.name[n]] = \
                                                data.hysteresis_model_effort[n]
    # commanded positions                                            
    @property
    def commanded_position(self):
        return self._commanded_position.values()                                            

    def commandPos_callback(self, data):
        with self._commandPos_lock:
            if data.gravity_model_effort:
                for n in xrange(0,len(data.name)):
                    self._commanded_position[data.name[n]] = \
                                                data.commanded_position[n]                                            
                                                
    # commanded torques                                           
    @property
    def commanded_torque(self):
        return self._commanded_torque.values()                                            

    def commandTor_callback(self, data):
        with self._commandTor_lock:
            if data.gravity_model_effort:
                for n in xrange(0,len(data.name)):
                    self._commanded_torque[data.name[n]] = \
                                                data.commanded_effort[n]    
                                                
    # all gravity torques                                           
    @property
    def all_gravity_torque(self):
        return self._all_gravity_torque.values()                                            

    def allGrav_callback(self, data):
        with self._allGrav_lock:
            if data.gravity_model_effort:
                for n in xrange(0,len(data.name)):
                    self._all_gravity_torque[data.name[n]] = \
                                                data.gravity_only[n]                                                  
                                                
                                                
     # crosstalk torques                                           
    @property
    def crosstalk_torque(self):
        return self._crosstalk_torque.values()                                            

    def crossTor_callback(self, data):
        with self._crossTor_lock:
            if data.gravity_model_effort:
                for n in xrange(0,len(data.name)):
                    self._crosstalk_torque[data.name[n]] = \
                                                data.crosstalk_model_effort[n]                                                 
                                                                                             
     # hysteresis state                                          
   # @property
    #def hysteresis_state(self):
     #   return self._hysteresis_state.values()                                            

    #def hystState_callback(self, data):
     #   with self._hystState_lock:
      #      if data.gravity_model_effort:
       #         self._hysteresis_state[data.name] = \
        #                                        data.hystState                                                  
