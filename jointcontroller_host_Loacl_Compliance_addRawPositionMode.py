#!/usr/bin/env python
#Kim Oakes added raw position mode Dec 21, 2020 -Warning:advanced control mode - does not implement safety features when commanding position
import roslib
...
function string getControlMode()
end object
...
class Baxter_impl(object):
    ...
    def setControlMode(self, mode):
        ...
        if mode == self.MODE_POSITION:
            ...
            self.setJointCommand('left',[0]*7)
            self.setJointCommand('right',[0]*7)
        elif mode == self.MODE_RAW_POSITION:
            self._left.exit_control_mode()
            self._right.exit_control_mode()
            # set command to current joint positions
            self.setJointCommand('left',self._jointpos[0:7])
    ...
    def command_worker(self):
        while self._running:
            ...
            elif (self._mode == self.MODE_RAW_POSITION):
                self._left.set_joint_positions(self._l_joint_command,True)
                self._right.set_joint_positions(self._r_joint_command,True)
            while (time.time() - t1 < 0.01):
                # idle
                time.sleep(0.001)
